...
def process_code_line(global_context: GlobalContext, line: str, snapshot: dict):
    def calc_val(m1):
        return eval(f'str({m1.group("expression")})', globals(), snapshot)

    m = re.search("\\$\\{(?P<expression>[^}]*)\\}", line)
    if m is None:
        return line

    return process_code_line(
        global_context, re.sub(CODE_USE_VARIABLE_RE.pattern, calc_val, line), snapshot
    )
...
def main():
    ...
    with open("program.tup") as file:
        text = file.read()
        result = compile(text)

    with open("program.tu", "w") as file:
        file.write(result)
        file.flush()
